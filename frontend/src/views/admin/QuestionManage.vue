<template>
  <AdminLayout title="提问管理">
    <div class="question-manage">
      <div class="toolbar">
        <el-input
          v-model="keyword"
          placeholder="搜索ID、标题、内容..."
          clearable
          style="width: 300px"
          @clear="handleSearch"
      ...
      <el-table :data="questionList" border stripe style="width: 100%" v-loading="loading" :row-class-name="rowClassName">
        <template #empty><el-empty description="暂无提问数据" /></template>
      ...
      <BanDialog v-model="banVisible" :user-id="banUserId" :user-nickname="banUserNickname" @done="fetchQuestions" />
    </div>
  </AdminLayout>
</template>
...
import type { Question, PageResult } from '@/types'
...
function rowClassName({ row }: { row: Question }) {
...
async function fetchQuestions() {
  loading.value = true
  try {
    const data = await adminApi.getQuestionList({
...
function handleToggle(row: Question) {
  const action = row.isOffline ? '上架' : '下架'
  ElMessageBox.confirm(`确定要${action}「${row.title}」吗？`, '提示', {
    confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning',
  }).then(async () => {
    await adminApi.offlineQuestion(row.id)
    ElMessage.success(`${action}成功`)
    await fetchQuestions()
  }).catch(() => {})
}

function handleBan(row: Question) {
  banUserId.value = row.userId
  banUserNickname.value = row.userNickname
  banVisible.value = true
}

watch([pageNum, pageSize], () => fetchQuestions())
onMounted(() => fetchQuestions())
</script>

<style scoped>
.question-manage { background: #fff; padding: 20px; border-radius: 4px; }
